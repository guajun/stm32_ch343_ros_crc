...
#define TX_BUFFER_SIZE (HEADER_SIZE + 80 + 2)
/* USER CODE END PD */
...
int main(void)
{
  ...
  MX_USART3_UART_Init();
  /* USER CODE BEGIN 2 */
  appendCRC16CheckSum(tx_data, 4+2+1+2);
  appendCRC16CheckSum(tx_data, TX_BUFFER_SIZE);
  HAL_UART_Transmit_DMA(&huart3, (uint8_t *)tx_data, TX_BUFFER_SIZE);
  /* USER CODE END 2 */
